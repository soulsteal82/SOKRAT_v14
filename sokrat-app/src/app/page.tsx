...
export default function Home() {
  ...
  const [tripCounter, setTripCounter] = useState(42);
  const [inputBatchSize, setInputBatchSize] = useState<number>(1);

  // Trip creation state
  const [newTripScope, setNewTripScope] = useState<TripScope>("FULL");
  const [newTripExternalDriver, setNewTripExternalDriver] = useState(false);
  const [newTripExternalInspector, setNewTripExternalInspector] = useState(false);
  const [newTripDriver, setNewTripDriver] = useState("");
  const [newTripDriverId, setNewTripDriverId] = useState("");
  const [newTripDriverPhone, setNewTripDriverPhone] = useState("");
  const [newTripIsExternal, setNewTripIsExternal] = useState(false);
  const [newTripPlate, setNewTripPlate] = useState("");
  const [newTripTrailerType, setNewTripTrailerType] = useState(TRAILER_TYPES[0]);
  const [newTripOwnership, setNewTripOwnership] = useState<"RENTED" | "OWNED">("OWNED");
...
function getAssetCustody(state: string | null | undefined, selectedTask?: any): string {
  ...
  const generateDriverLink = () => {
    const link = `https://app.sokrat.ae/driver/${manifest.manifest_group_id}`;
    setManifest(prev => ({ ...prev, driverPortalLink: link }));
    navigator.clipboard.writeText(link);
    alert(`✅ Driver link copied to clipboard!\n\n${link}\n\nSend this link to the external driver. They can open it in their phone browser to view trip details and confirm delivery.`);
  };

  // Generate external inspector link
  const generateInspectorLink = () => {
    const link = `https://app.sokrat.ae/inspector/${manifest.manifest_group_id}`;
    setManifest(prev => ({ ...prev, inspectorPortalLink: link }));
    navigator.clipboard.writeText(link);
    alert(`✅ Inspector link copied to clipboard!\n\n${link}\n\nSend this link to the external inspector. They can open it in their phone browser to confirm delivery receipt.`);
  };

  function handleInitialiseNewTrip() {
    // Validate required fields
    if (!newTripDriver || !newTripDriverPhone || !newTripPlate) {
      setError("Please fill in Driver Name, Phone, and Plate Number");
      return;
    }

    const nextTripId = tripCounter + 1;
    setTripCounter(nextTripId);
    const newManifestGroupString = `ICAD-YAS-ALM-260706-${String(nextTripId).padStart(3, "0")}`;

    setManifest({
      manifest_group_id: newManifestGroupString,
      fleet_carrier_id: "AL FARIS HEAVY LOGISTICS",
      geofence_verified: false,
      factory_gate_out_actual: new Date().toISOString(),
      site_gate_in_target: new Date(Date.now() + 7200000).toISOString(),
      scope: newTripScope,
      externalDriver: newTripExternalDriver,
      externalInspector: newTripExternalInspector,
      driver: {
        name: newTripDriver,
        id: newTripDriverId || `DRV-${String(nextTripId).padStart(3, "0")}`,
        phone: newTripDriverPhone,
        email: "",
        rating: 0,
        totalTrips: 0,
        isExternal: newTripIsExternal
      },
      vehicle: {
        plateNumber: newTripPlate,
        trailerType: newTripTrailerType as "FLATBED" | "LOWBOY" | "STEP_DECK" | "REMOVABLE_GOOSENECK",
        ownership: newTripOwnership,
      },
      siteInspector: {
        name: newInspectorName || "TBD",
        phone: newInspectorPhone || "TBD",
    ...
    const freshBatch = generateInitialBatch(newManifestGroupString, inputBatchSize);
    ...
    setShowInspectorLink(false);

    // Reset form fields
    setNewTripDriver("");
    setNewTripDriverId("");
    setNewTripDriverPhone("");
    setNewTripIsExternal(false);
    setNewTripPlate("");
    setNewTripTrailerType(TRAILER_TYPES[0]);
    setNewTripOwnership("OWNED");
    setNewInspectorName("");
    setNewInspectorPhone("");
    setNewInspectorEmail("");
    setNewTripScope("FULL");
    setNewTripExternalDriver(false);
    setNewTripExternalInspector(false);
  }
...
>
  ✅ Return to My Tasks
        ...
        {profile === "DISPATCHER" && (isTripComplete() || assets.length === 0) && (
          <div className="border-t border-slate-800/80 pt-3 mt-2 space-y-3">
            ...
            {/* Scope Selection */}
            <div className="bg-slate-950 p-3 border border-slate-800 rounded-lg space-y-2">
              <span className="text-[8px] uppercase text-slate-400 font-bold">Service Scope</span>
              <div className="grid grid-cols-2 gap-2">
                <div className="col-span-2">
                  <label className="text-[9px] text-slate-400">Trip Type</label>
                  <select
                    value={newTripScope}
                    onChange={(e) => setNewTripScope(e.target.value as TripScope)}
                    className="w-full bg-slate-900 border border-slate-700 rounded p-1 text-xs text-slate-200"
                  >
                    <option value="FULL">Full Service (Dispatch → Installation)</option>
                    <option value="FACTORY_ONLY">Factory Only (Ends at Dispatch)</option>
                    <option value="DELIVERY_ONLY">Delivery Only (Ends at Gate)</option>
                  </select>
                  <p className="text-[7px] text-slate-500 mt-1">
                    {newTripScope === "FULL" && "🔵 Full tracking: Factory → Installation"}
                    {newTripScope === "FACTORY_ONLY" && "🏭 Ends at DISPATCHED (Client pickup)"}
                    {newTripScope === "DELIVERY_ONLY" && "🚚 Ends at ARRIVED_AT_GATE (Client handles installation)"}
                  </p>
                </div>
                <div>
                  <label className="flex items-center gap-2 text-[9px] text-slate-400 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={newTripExternalDriver}
                      onChange={(e) => setNewTripExternalDriver(e.target.checked)}
                      className="accent-cyan-500"
                    />
                    External Driver (Send link)
                  </label>
                </div>
                <div>
                  <label className="flex items-center gap-2 text-[9px] text-slate-400 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={newTripExternalInspector}
                      onChange={(e) => setNewTripExternalInspector(e.target.checked)}
                      className="accent-cyan-500"
                    />
                    External Inspector (Send link)
                  </label>
                </div>
              </div>
            </div>
            ...
            {/* Driver Details */}
            <div className="bg-slate-950 p-3 border border-slate-800 rounded-lg space-y-2">
              <span className="text-[8px] uppercase text-slate-400 font-bold">Driver Details</span>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-[9px] text-slate-400">Driver Name *</label>
                  <input
                    type="text"
                    value={newTripDriver}
                ...
                <div>
                  <label className="text-[9px] text-slate-400">Driver ID</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-[9px] text-slate-400">Phone *</label>
                  <input
                    type="text"
                    value={newTripDriverPhone}
                    onChange={(e) => setNewTripDriverPhone(e.target.value)}
                    className="w-full bg-slate-900 border border-slate-700 rounded p-1 text-xs text-slate-200"
                ...
                <div className="flex items-center">
                  <label className="flex items-center gap-2 text-[9px] text-slate-400 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={newTripIsExternal}
                      onChange={(e) => setNewTripIsExternal(e.target.checked)}
                      className="accent-cyan-500"
                    />
                    External Driver
                  </label>
            ...
            {/* Vehicle Details */}
            <div className="bg-slate-950 p-3 border border-slate-800 rounded-lg space-y-2">
              <span className="text-[8px] uppercase text-slate-400 font-bold">Vehicle Details</span>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-[9px] text-slate-400">Plate Number *</label>
                  <input
                    type="text"
                    value={newTripPlate}
                    onChange={(e) => setNewTripPlate(e.target.value)}
                    className="w-full bg-slate-900 border border-slate-700 rounded p-1 text-xs text-slate-200"
                    placeholder="A-12345"
                  />
                </div>
                <div>
                  <label className="text-[9px] text-slate-400">Trailer Type</label>
                  <select
                    value={newTripTrailerType}
                    onChange={(e) => setNewTripTrailerType(e.target.value)}
                    className="w-full bg-slate-900 border border-slate-700 rounded p-1 text-xs text-slate-200"
                  >
                ...
                <div>
                  <label className="text-[9px] text-slate-400">Ownership</label>
                  <select
                    value={newTripOwnership}
                    onChange={(e) => setNewTripOwnership(e.target.value as "RENTED" | "OWNED")}
                    className="w-full bg-slate-900 border border-slate-700 rounded p-1 text-xs text-slate-200"
                  >
            ...
            <div className="bg-slate-950 p-3 border border-slate-800 rounded-lg space-y-2">
              <span className="text-[8px] uppercase text-slate-400 font-bold">👷 Site Inspector</span>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-[9px] text-slate-400">Inspector Name</label>
                  <input
                    type="text"
                ...
                <div className="col-span-2">
                  ...
                  <input
                    type="email"
                    value={newInspectorEmail}
                    onChange={(e) => setNewInspectorEmail(e.target.value)}
                    className="w-full bg-slate-900 border border-slate-700 rounded p-1 text-xs text-slate-200"
                    placeholder="inspector@site.ae"
                  />
                </div>
              </div>
            </div>
            ...
            {/* Create Trip Button */}
            <button
              onClick={handleInitialiseNewTrip}
              className="w-full bg-cyan-600 hover:bg-cyan-500 text-slate-950 font-bold py-2 rounded text-xs uppercase transition"
            >
              🚀 Create Trip
            </button>
          </div>
